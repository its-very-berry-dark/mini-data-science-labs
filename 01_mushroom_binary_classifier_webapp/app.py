...
def main():
    ...
    def plot_metrics(metrics_list):
        if "Confusion Matrix" in metrics_list:
            ...
            disp = ConfusionMatrixDisplay.from_estimator(model, x_test, y_test, display_labels=class_names, cmap="Greens")
    ...
    if classifier == "Random Forest":
        st.sidebar.subheader("Model Hyperparameters")
        n_estimators = int(st.sidebar.number_input("The number of tress in the forest", 100, 5000, step=10, key="n_estimators"))
        max_depth = int(st.sidebar.number_input("the maximum depth of the tree", 1, 20, step=1, key="max_depth"))
    ...
    if st.sidebar.checkbox("Show raw data", False):
        ...
        st.markdown("Overall datasets: ")
        st.write(df)


    num_rows, num_cols = df.shape

    with st.expander("𓍊𓋼𓍊𓋼𓍊  Data Summary  𓍊𓋼𓍊𓋼𓍊"):  
        st.markdown(f"""
        **Mushroom Dataset**  
        • Rows: **{num_rows} observed mushrooms**  
        • Columns: **{num_cols} features**  

        **Model Used:** {classifier}  
